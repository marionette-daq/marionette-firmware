/*! \file fetch_dac124s085.c
 * Marionette fetch_dac124s085 routines
 * @defgroup fetch_dac124s085 Fetch external ADC
 * @{
 */
...
static bool fetch_dac124_config_cmd(BaseSequentialStream * chp, char * cmd_list[], char * data_list[]);
static bool fetch_dac124_exchange_cmd(BaseSequentialStream * chp, char * cmd_list[], char * data_list[]);
static bool fetch_dac124_reset_cmd(BaseSequentialStream * chp, char * cmd_list[], char * data_list[]);
//static bool fetch_dac124_help_cmd(BaseSequentialStream * chp, char * cmd_list[], char * data_list[]);


/*static fetch_command_t fetch_dac124_commands[] = {
    { fetch_dac124_exchange_cmd,  "exchange",  "TX/RX bytes\n" \
                                            "Usage: exchange(<dev>,<base>,<byte 0>,[...,<byte n>])" },
    { fetch_dac124_config_cmd,    "config",    "Configure SPI driver\n" \
    ...
    { fetch_dac124_reset_cmd,     "reset",     "Reset SPI driver\n" \
                                            "Usage: reset(<dev>)" },
    { fetch_dac124_help_cmd,      "help",      "SPI command help" },
...
/*  static uint8_t tx_buffer[MAX_SPI_BYTES];
  ...
  static uint16_t rx_buffer[MAX_SPI_BYTES];
  ...
  for( int i = 0; i < MAX_SPI_BYTES && data_list[i+2] != NULL; i++ )
  {
    ...
    else if( byte_value < 0 || byte_value > 0xffff ) //changed from 0xff to use 16bit
    {
  ...
  util_message_hex_uint16( chp, "rx", rx_buffer, byte_count);
...
static bool fetch_dac124_help_cmd(BaseSequentialStream * chp, char * cmd_list[], char * data_list[])
{
  ...
  fetch_display_help(chp, fetch_dac124_commands);
	return true;
}
*/
...
/*bool fetch_dac124_dispatch(BaseSequentialStream * chp, char * cmd_list[], char * data_list[])
{
  if( spi_init_flag )
  {
    fetch_dac124_init(chp);
  }
  return fetch_dispatch(chp, fetch_dac124_commands, cmd_list[FETCH_TOK_SUBCMD_0], cmd_list, data_list);
}*/
